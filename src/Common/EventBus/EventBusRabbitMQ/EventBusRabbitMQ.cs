...
namespace EventBusRabbitMQ
{
    public class EventBusRabbitMQ : IEventBus, IDisposable
    {
        ...
        public void Publish(IntegrationEvent @event)
        {
            ...
            Log.Verbose("Creating RabbitMQ channel to publish event: {EventId} ({EventName})", @event.Id, eventName);

            using (var channel = this._persistentConnection.CreateModel())
            {

                Log.Verbose("Declaring RabbitMQ exchange to publish event: {EventId}", @event.Id);
                ...
                policy.Execute(() =>
                {
                    ...
                    Log.Verbose("Publishing event to RabbitMQ: {EventId}", @event.Id);
        ...
        {
            Log.Verbose("Starting RabbitMQ basic consume");
        ...
        private IModel CreateConsumerChannel()
        {
            ...
            Log.Verbose("Creating RabbitMQ consumer channel");
        ...
        {
            Log.Verbose("Processing RabbitMQ event: {EventName}", eventName);

            if (this._subsManager.HasSubscriptionsForEvent(eventName))
            {
                using (var scope = this._autofac.BeginLifetimeScope(this.AUTOFAC_SCOPE_NAME))
                {
